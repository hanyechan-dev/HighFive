...
public class WebClientUtil {
	...
	public AiConsultingCreateDto postRequestToPython(List<Map<String, String>> promptList,
	...
	public String sendEmbeddingRequestJobPosting(String metadata, List<MultipartFile> images) {
		...
		return webClient.post().uri("/embedding-job-posting") // 파이썬 엔드포인트
				.contentType(MediaType.MULTIPART_FORM_DATA).body(BodyInserters.fromMultipartData(builder)).retrieve()
				.bodyToMono(String.class).block();
	}

	public String sendEmbeddingRequestMember(String data) {

		return webClient.post().uri("/embedding-member").bodyValue(data).retrieve().bodyToMono(String.class).block();
