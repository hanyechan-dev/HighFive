...
public class TextBuilder {
	...
	public String getEducationStringForEmbedding(Education education) {
		StringBuilder sb = new StringBuilder();
		sb.append("▶ 학력\n");
		sb.append(String.format("- 학교명: %s / 전공: %s / GPA: %s / 재학기간: %s ~ %s\n", education.getSchoolName(),
				education.getMajor(), education.getGpa(), education.getEnterDate(), education.getGraduateDate()));

		return sb.toString();

	}

	public String getCareerStringForEmbedding(Career career) {
		StringBuilder sb = new StringBuilder();
		sb.append("\n▶ 경력\n");
		sb.append(String.format("- 기업명: %s / 직무명: %s / 부서명: %s / 직급: %s / 재직기간: %s ~ %s\n", career.getCompanyName(),
				career.getJob(), career.getDepartment(), career.getPosition(), career.getStartDate(),
				career.getEndDate()));

		return sb.toString();
	}

	public String getCertificationStringForEmbedding(Certification certification) {
		StringBuilder sb = new StringBuilder();
		sb.append("\n▶ 자격증\n");
		if (certification.getGrade() == null) {
			sb.append(String.format("- 자격증명: %s / 점수: %s / 취득일: %s\n", certification.getCertificationName(),
					certification.getScore(), certification.getAcquisitionDate()));
		} else {
			sb.append(String.format("- 자격증명: %s / 등급: %s / 취득일: %s\n", certification.getCertificationName(),
					certification.getGrade(), certification.getAcquisitionDate()));
		}

		return sb.toString();
	}

	public String getLanguageTestStringForEmbedding(LanguageTest languageTest) {
		StringBuilder sb = new StringBuilder();
		sb.append("\n▶ 어학시험\n");
		if (languageTest.getGrade() == null) {
			sb.append(String.format("- 언어: %s / 시험: %s / 점수: %s / 취득일: %s\n", languageTest.getLanguageType(),
					languageTest.getTestName(), languageTest.getScore(), languageTest.getAcquisitionDate()));
		} else {
			sb.append(String.format("- 언어: %s / 시험: %s / 등급: %s / 취득일: %s\n", languageTest.getLanguageType(),
					languageTest.getTestName(), languageTest.getGrade(), languageTest.getAcquisitionDate()));
		}

		return sb.toString();
	}

	public String getCareerDescriptionStringForEmbedding(CareerDescription careerDescription) {
		StringBuilder sb = new StringBuilder();
		sb.append("\n▶ 경력기술서\n");
		if (careerDescription.getCareerDescriptionContents() == null
				|| careerDescription.getCareerDescriptionContents().isEmpty()) {
			sb.append("- 상세 내용 없음\n");
		} else {
			for (CareerDescriptionContent cdc : careerDescription.getCareerDescriptionContents()) {
	...
	public String getCoverLetterStringForEmbedding(CoverLetter coverLetter) {
		StringBuilder sb = new StringBuilder();
		sb.append("\n▶ 자기소개서\n");
		if (coverLetter.getCoverLetterContents() == null
				|| coverLetter.getCoverLetterContents().isEmpty()) {
			sb.append("- 상세 내용 없음\n");
		} else {
			for (CoverLetterContent clc : coverLetter.getCoverLetterContents()) {
				sb.append(String.format("- 항목: %s, 내용: %s\n", clc.getItem(), clc.getContent()));
			}
		}
