...
public class ChatServiceImpl implements ChatService {
	...
	public ChatResponseDto createMessage(ChatRequestDto chatRequestDto) {
		...
		ChatContent newMessage = ChatContent.builder().user(user).chatRoom(chatRoom).content(content).build();

		ChatContent chatContent = chatContentRepository.save(newMessage);

		ChatResponseDto chatResponseDto = ChatResponseDto.builder().chatRoomId(chatRequestDto.getChatRoomId())
				.contentId(chatContent.getId()).senderId(chatRequestDto.getSenderId())
				.name(chatContent.getUser().getName()).content(chatRequestDto.getContent())
				.createdAt(chatContent.getCreatedTime()).build();
	...
	public List<ChatResponseDto> readChatRoomList(Long id) {
		...
		if (chatRooms != null) {
			...
			return chatRooms.stream().map(chatRoom -> {
				User otherUser = chatRoom.getUser1().getId().equals(id) ? chatRoom.getUser2() : chatRoom.getUser1();

				String name;

				if (otherUser.getUserType() == UserType.기업회원) {
				...
				return ChatResponseDto.builder().chatRoomId(chatRoom.getId()).receiverId(otherUser.getId()).name(name)
						.build();
	...
	public List<ChatResponseDto> readMessagesList(Long roomId) {
		...
		if (chatRooms != null) {
			dtoList = chatRooms.getChatContents().stream()
					.map(chatContent -> ChatResponseDto.builder().chatRoomId(roomId).contentId(chatContent.getId())
							.senderId(chatContent.getUser().getId()).name(chatContent.getUser().getName())
							.content(chatContent.getContent()).createdAt(chatContent.getCreatedTime()).build())
					.collect(Collectors.toList());
		} else {
