...
const PaymentModal = ({ paymentAmount ,onClick ,onClose}: PaymentModalProps) => {
    ...
    return (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
            <div className={` overflow-y-auto overflow-x-hidden bg-white rounded-lg w-[512px] font-roboto`}>
                ...
                <div className="flex justify-between mr-6">
                    <Button color={"white"} size={"s"} disabled={false} text={"취소"} type={"button"} onClick={()=>{onClose(false)}} />
                    <Button color={"theme"} size={"s"} disabled={false} text={"결제 진행"} type={"button"} onClick={onClick}/>
                </div>
            </div>
        </div >
