...
interface RequestDetailModalProps {
    ...
    setShowRequestDetailModal : (showRequestDetailModal : boolean) => void
}

const RequestDetailModal = ({requestDetailDto, isRequestDetailDtoLoading ,setShowRequestDetailModal} : RequestDetailModalProps) => {
    ...
    // ⚠️ 데이터 없으면 로딩 표시
    if (isRequestDetailDtoLoading || !requestDetailDto) {
        return (
            <CommonModal size="l" onClose={onClose}>
                <LoadingSpinner message="컨설팅 생성중..." />
            </CommonModal>
        );
    }

    // ✅ 이제 안전하게 destructuring
    const {
        requestResponseDto: {
            targetCompanyName,
            targetJob,
        ...
        aiConsultingResponseDto: { aiConsultingContentResponseDtos },
    } = requestDetailDto;

    const [checkedDocument, setCheckedDocument] = useState(
        documentTypeEnum[0].value
    );
    ...
    const aiConsultingContentForCareerDescriptionResponseDtos =
        aiConsultingContentResponseDtos.filter(
            (item) => item.documentType === "경력기술서"
        );

    const aiConsultingContentForCoverLetterResponseDtos =
        aiConsultingContentResponseDtos.filter(
            (item) => item.documentType === "자기소개서"
        );
        ...
        <CommonModal size="l" onClose={onClose}>
            ...
            {checkedDocument === "이력서" && (
                <ExternalBox>
                    ...
                    {checkedResume === "학력사항" &&
                        educationResponseDtos.length > 0 &&
                        educationResponseDtos.map((educationResponseDto) => (
                            <EducationInfo
                    ...
                    {checkedResume === "경력사항" &&
                        careerResponseDtos.length > 0 &&
                        careerResponseDtos.map((careerResponseDto) => (
                            <CareerInfo
                    ...
                    {checkedResume === "자격증" &&
                        certificationResponseDtos.length > 0 &&
                        certificationResponseDtos.map((certificationResponseDto) => (
                            <CertificationInfo
                    ...
                    {checkedResume === "어학" &&
                        languageTestResponseDtos.length > 0 &&
                        languageTestResponseDtos.map((languageTestResponseDto) => (
                            <LanguageTest
            ...
            {checkedDocument === "경력기술서" &&
                (careerDescriptionResponseDto ? (
                    ...
                    <EmptyState
                        title={"해당 컨설팅은 경력기술서를 포함하지 않았습니다."}
                        text={""}
                    />
                ))}
            ...
            {checkedDocument === "자기소개서" &&
                (coverLetterResponseDto ? (
                    <>
                        ...
                        <AiConsulting
                            aiConsultingContentResponseDtos={
                                aiConsultingContentForCoverLetterResponseDtos
                            }
                            consultingType={consultingType}
                        />
                    </>
                    ...
                    <EmptyState
                        title={"해당 컨설팅은 자기소개서를 포함하지 않았습니다."}
                        text={""}
                    />
                ))}
        </CommonModal>
