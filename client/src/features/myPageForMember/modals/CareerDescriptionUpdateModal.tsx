...
import type { CareerDescriptionContentCreateDto, CareerDescriptionResponseDto, CareerDescriptionUpdateDto } from "../props/myPageForMemberProps";

const convertToUpdateDto = (responseDto: CareerDescriptionResponseDto): CareerDescriptionUpdateDto => {
    return {
        id: responseDto.id,
...
interface CareerDescriptionUpdateModalProps {
    careerDescriptionResponseDto: CareerDescriptionResponseDto;
    onUpdate: (careerDescriptionUpdateDto: CareerDescriptionUpdateDto) => void;
    setShowModalType: (modalType: string) => void
}

const CareerDescriptionUpdateModal = ({
    careerDescriptionResponseDto,
    onUpdate,
    setShowModalType,
}: CareerDescriptionUpdateModalProps) => {

    const [responseDto, setResponseDto] = useState<CareerDescriptionResponseDto>(careerDescriptionResponseDto);

    useEffect(() => {
        setResponseDto(careerDescriptionResponseDto);
    }, [careerDescriptionResponseDto]);
    ...
    const [newContent, setNewContent] = useState<CareerDescriptionContentCreateDto>({
        item: "",
        content: ""
    });
    ...
    const onClickCancelButton = () => {
        setResponseDto(careerDescriptionResponseDto); // 초기화
        setIsAddMode(false);
        setNewContent({ item: "", content: "" });
        ...
        <>
            ...
            <ExternalBox>
                ...
                {isAddMode && (
                    <InternalBox >
                        <div className="mt-[-24px]">
                            <Input label={""} placeholder={"경력기술서 항목을 입력해주세요."} size={"ibl"} disabled={false} type={"text"} value={newContent.item} setValue={setNewItem} />
                        </div>
                        <div className="mt-[-24px]">
                            <TextArea size={"ibl"} label={""} placeholder={"경력기술서 내용을 입력해주세요."} disabled={false} value={newContent.content} setValue={setNewContentValue} />
                        </div>
                        <div className="flex justify-end mr-6">
...
export default CareerDescriptionUpdateModal;
